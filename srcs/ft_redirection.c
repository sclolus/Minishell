...
/*   Updated: 2017/04/28 18:29:33 by sclolus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < prefix_n)
	{
		if (OR_PARSER_N(MULTIPLY_PARSER_N(cmd_prefix, i), 1)->retained)
			f_redirec[ft_get_redirec_number(AND_PARSER_N(
				...
				(AND_PARSER_N(OR_PARSER_N(MULTIPLY_PARSER_N(cmd_prefix, i), 1), 0));
		i++;
	}
	return (1);
}
									...
									, const t_redirec *f_redirec)
{
	...
	while (i < postfix_n)
	{
		if (OR_PARSER_N(MULTIPLY_PARSER_N(cmd_postfix, i), 1)->retained)
			f_redirec[ft_get_redirec_number(AND_PARSER_N(
				OR_PARSER_N(MULTIPLY_PARSER_N(cmd_postfix, i), 1), 0))]
				(AND_PARSER_N(OR_PARSER_N(MULTIPLY_PARSER_N(cmd_postfix, i), 1), 0));
		i++;
	}
	return (1);
}
